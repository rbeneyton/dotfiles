...
$(GIT_INSTALL) : | $(GCC_INSTALL) $(UTILS)
	...
	rm -rf $(SRC)
	mkdir -p $(SRC)
	wget https://www.kernel.org/pub/software/scm/git/git-2.48.1.tar.xz -O $(TAR)
...
$(NEOVIM_INSTALL) : | $(GCC_INSTALL) $(UTILS)
	...
	rm -rf $(SRC)
	git clone --branch release-0.10 --single-branch --depth 50 https://github.com/rbeneyton/neovim.git $(SRC)
